...
def bayeslearn(x_train: np.array, y_train: np.array):
    ...
    """
    x_labeld1 = x_train[y_train == 1]
    x_labeld_1 = x_train[y_train == -1]
    ppos = np.mean(x_labeld1, axis=0)
    pneg = np.mean(x_labeld_1, axis=0)
    allpos = len(x_labeld1) / x_train.shape[0]
...
def bayespredict(allpos: float, ppos: np.array, pneg: np.array, x_test: np.array):
    ...
    for x in x_test:
        val1 = np.log2(allpos)
        val_1 = np.log2(1 - allpos)
        for ind, val in enumerate(x):
            if ppos[ind] != 0:
                val1 += np.log2(ppos[ind]) if val == 1 else np.log2(1 - ppos[ind])
            if pneg[ind] != 0:
                val_1 += np.log2(pneg[ind]) if val == 1 else np.log2(1 - pneg[ind])
        if val1 > val_1:
            y_pred.append(1)
        else:
            y_pred.append(-1)
    return np.asarray(y_pred).reshape(len(y_pred), 1)
